...
async function preloadFirmwareRuntimeState() {
  ...
  try {
    ...
    // Get the complete status (including the first frame LED in renderer.lastM370),
    // Instead of runtimeOnly/noFrame summary and apply it with skipFrame:false,
    // Let the fundamental matrix preview be filled from the first frame during the loading animation.
...
async function restoreScrollTextFromFirmware(source = "post_boot", options = {}) {
  ...
  try {
    ...
    // D10/E4: Detect sanitize truncation (overlong third-party text); truncation recovery never binds
    // framesTimelineId, even if the frame numbers happen to be consistent.
...
async function runPostBootDeferredReads(bootOk = false) {
  ...
  // Otherwise those slow requests will make /api/scroll/meta look like nothing happened.
...
async function bootstrapWebUi() {
  ...
  try {
    ...
    // Firmware starts reading at the beginning of the minimum display window (now includes the first frame LED),
    // Let it overlap with the otherwise idle waiting time. The first frame matrix + runtime state will be in
    // The loading animation is still applied when showing, so the loader closes/reveals the page.
    // The base matrix preview is populated without leaving the shutdown process waiting for the network.
    const runtimePingPromise = preloadFirmwareRuntimeState()
      ...
      .catch((err) => {
        if (shouldLogApiError()) log(`runtime 状态读取失败：${err.message || err}`, "error");
      });
    ...
    // Make sure the runtime snapshot (and bootOk) is stable first,
    // Then start the delayed reading and status polling that rely on it.
