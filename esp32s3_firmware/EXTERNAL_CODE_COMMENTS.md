...
| `rina_protocol.py` | 735 行 | 协议兼容层：支持原版二进制 UDP、微信小程序文本协议、M370 物理矩阵扩展、WebUI 管理命令。 |
| `esp32s3_network.py` | 621 行 | ESP32-S3 原生网络层：同时启用 AP/STA、UDP 服务、HTTP 控制台/API，替代旧 UART Wi-Fi 桥。 |
| `webui_runtime.py` | 378 行 | WebUI 动画运行时：把滚动文字和 Unity 时间轴播放从浏览器计时迁移到固件主循环。 |
| `main.py` | 1107 行 | 主应用入口和调度循环：连接状态、按键路由、显示覆盖层、电池服务、WebUI、协议、自动换脸。 |
