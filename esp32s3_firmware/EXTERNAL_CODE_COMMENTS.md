...
- 源 ZIP：`esp32s3_firmware.zip`
- 本次输出 ZIP：`esp32s3_firmware_ma_web_to_m_fix.zip`
- 注释生成时间：2026-04-26
- 注释方式：**不改动源码文件**；所有解释都集中在本 `.md` 文件。
- 行号说明：下面的行号基于本次上传 ZIP 解压后的当前源码。


## 0. 本次修复：网页控制自动回到 M 模式

本次修改目标：修复 A/M 自动换脸状态。只要网页控制端或网络协议发送**控制类命令**，固件会立即把 `AppState.auto` 置为 `False`，也就是回到 **M 模式**，停止自动换脸。

关键改动：

| 文件 | 修改点 | 作用 |
|---|---|---|
| `main.py` | `exit_manual_control_from_network()` 现在同时执行 `self.state.auto = False` | 所有调用该入口的 WebUI/网络控制都会强制回到 M。 |
| `main.py` | `handle_webui_runtime_command()` 对除 `runtimeStatus` 外的滚动文字/Unity 时间轴命令先调用 `exit_manual_control_from_network()` | 滚动文字、timeline begin/chunk/play/preview/stop/clear 都会关闭 A 自动轮播。 |
| `rina_protocol.py` | `setManualMode|0` / `manualControlMode|0` 改为走 `exit_manual_control_from_network()` | WebUI 手动模式按钮关闭时也不会残留 A 自动轮播。 |
| `rina_protocol.py` | 版本号改为 `1.6.1-esp32s3-native-ma-web-to-m` | 方便在 WebUI/协议读取版本时确认已更新。 |
| `esp32s3_network.py` | `/api/status` 固件标识改为 `1.6.1-esp32s3-native-ma-web-to-m` | 浏览器状态页可看到新构建。 |
| `webui_index.html.gz` | 页面标题/说明更新为 `1.6.1-MA-web-to-M` | 页面说明与新 M/A 行为一致。 |

设计说明：网页触发的 M 模式切换只改运行时状态，不在每个 Web 命令上写入 `settings.json`，避免滚动文字/Unity 时间轴等高频命令反复写 Flash。实体 B3 的 M/A 切换仍按原逻辑保存设置。
...
当前 `main.py` 的启动横幅写明本构建是 **1.6.1 M/A web-control-to-M / no MatrixDemo / no BadApple**：相关文件和配置仍保留在项目中，但主启动路径不导入 `matrix_demos.py`、`demo_faces.py` 或 BadApple part 模块，以降低启动时堆内存压力。

## 2. 文件职责索引

| 文件 | 行数/大小 | 外置注释摘要 |
|---|---:|---|
| `boot.py` | 15 行 | MicroPython 启动阶段执行的最小启动脚本；只做垃圾回收和启动日志，避免在 boot 阶段占用堆。 |
| `config.py` | 139 行 | 全局配置中心：按键轮询、亮度、动画、Battery ADC、充电检测、边缘闪烁、BadApple/演示相关常量。 |
| `app_state.py` | 120 行 | 运行期状态对象：AppState 保存界面/按键/覆盖层/网络控制状态，BatteryState 保存电池校准和历史。 |
| `settings_store.py` | 105 行 | 持久化设置读写层：将亮度、自动播放、间隔、电池校准和电量历史写入 JSON。 |
| `brightness_modes.py` | 57 行 | 把 UI 百分比亮度换算成物理 LED 每通道亮度上限。 |
| `buttons.py` | 220 行 | 实体按键输入层：GPIO 上拉、低电平有效、消抖、长按重复触发。 |
| `board.py` | 659 行 | 主硬件抽象层：ESP32-S3 GPIO2 驱动 370 颗 WS2812，定义不规则 22×18 虚拟矩阵映射和绘图基础函数。 |
| `board_370.py` | 163 行 | 较精简的 370 LED 板卡驱动备用实现；当前 main.py 使用 board.py，不直接使用本文件。 |
| `display_num.py` | 255 行 | 数码/图标/滚动文字渲染器：间隔、亮度、电池百分比/电压/时间、A/M、大号字、IP/SSID 滚动显示。 |
| `battery_runtime.py` | 139 行 | 电池续航/充电时间估算器：记录百分比变化速率，并按亮度/模式加权估算剩余时间。 |
| `battery_monitor.py` | 309 行 | 电池采样和校准逻辑：ADC 电压读取、1 秒均值窗口、百分比曲线、充电检测、颜色映射。 |
| `emoji_db.py` | 129 行 | 原版 RinaChanBoard 表情部件数据库：左右眼、嘴巴、脸颊 bitpack 数据和布局常量。 |
| `saved_faces_370.py` | 343 行 | 370 物理矩阵表情库：默认表情、WebUI 保存/重命名/删除/排序/锁定，JSON 作为固件端真值。 |
| `rina_protocol.py` | 737 行 | 协议兼容层：支持原版二进制 UDP、微信小程序文本协议、M370 物理矩阵扩展、WebUI 管理命令。 |
| `esp32s3_network.py` | 620 行 | ESP32-S3 原生网络层：同时启用 AP/STA、UDP 服务、HTTP 控制台/API，替代旧 UART Wi-Fi 桥。 |
| `webui_runtime.py` | 377 行 | WebUI 动画运行时：把滚动文字和 Unity 时间轴播放从浏览器计时迁移到固件主循环。 |
| `main.py` | 1122 行 | 主应用入口和调度循环：连接状态、按键路由、显示覆盖层、电池服务、WebUI、协议、自动换脸。 |
| `demo_faces.py` | 297 行 | 旧版内置 ASCII 表情帧；当前 no-demo 构建不在主启动路径导入，用于保留/迁移参考。 |
| `matrix_demos.py` | 472 行 | 旧版矩阵演示动画控制器；当前 main.py 中演示模式被禁用，不在主启动路径导入。 |
| `wifi_config.py` | 15 行 | Wi-Fi/AP 配置文件：STA SSID/密码、AP SSID/密码、HTTP/UDP 端口。 |
...
| 自动换脸 / A-M 模式 | `main.py` 的 `toggle_auto()`、`exit_manual_control_from_network()`，以及 `settings_store.py`, `saved_faces_370.py` |
...
## v1.6.2 WebUI / TXS0108E / 日志修复外置注释

### 1. `webui_index.html.gz`
- 修复 Wi-Fi 扫描结果行的内联 JavaScript 字符串引号错误。该错误会导致浏览器停止解析整段脚本，从而表现为 WebUI 按钮“无任何反应”。
- 重新压缩前移除了 HTML 解压内容末尾的 NUL 填充字节。
- 页面版本文字更新为 `1.6.2-webui-txs-logfix`。

### 2. `esp32s3_network.py`
- 新增 HTTP 请求路径日志：每次 GET/POST 都会打印 `>>> [HTTP GET] 请求路径: ...` 或对应方法。
- 新增静态文件打开日志：成功打开文件打印 `>>> [File] 成功打开文件: ...`，打开失败打印 `!!! [404 Error] 文件不存在或读取失败: ...`。
- 新增静态文件发送前后的 `gc.mem_free()` 内存日志，用于判断 `.gz` 静态资源发送是否造成内存压力。
- 所有 HTTP header/body/static chunk 发送都经过 `_safe_send()`，客户端提前断开、ECONNRESET、EPIPE 等会打印 `!!! [Socket Error] ...`。
- 静态文件按 1024 byte 分块发送，不一次性读取整个 `.gz` 文件。
- 增加 `/api/wifi/status`、`/api/wifi/scan`、`/api/wifi/save`，使 WebUI Wi-Fi 配网页按钮有后端响应。
- 增加静态资源 fallback，未来如果 WebUI 拆分出 `.js`、`.json.gz`、`.css` 文件，路径正确时可以直接分块发送；路径错误时会明确打印 404。

### 3. `webui_runtime.py`
- `handle_command()` 入口新增 `>>> [API Command] 收到前端指令: ...`。
- 命令处理主体加入 `try/except`，异常时打印 `!!! [API Crash] 处理前端指令时发生严重错误: ...` 并返回 `ERR:...`。

### 4. `board.py`
- 在 `NeoPixel` 初始化前先将 GPIO2 配置为输出低电平并保持 5 ms，降低 ESP32-S3 GPIO/RMT 接管瞬间通过 TXS0108E 被放大的启动脉冲。
- 新增 `_write_np_safely()`，在每次 `np.write()` 前后插入 280 us reset/settle gap。
- 上电后发送两次全黑帧，清除可能被 WS2812 锁存的启动毛刺帧。

### 5. `battery_monitor.py` / `main.py`
- 移除周期性的 `battery log: ...` 串口输出。
- 移除电池显示刷新时的 `battery display: ...` 串口输出。
- 电池数据仍然通过 `requestBattery`、`requestState` 和 WebUI 电池面板提供。

## v1.6.3 颜色 / Unity 帧序列 Assets 按需加载外置注释

### 修改目标

本版本针对 WebUI 中“颜色”和“Unity 时间轴”资源缺失、页面初始体积过大、资源长期驻留内存的问题进行调整。

### 新增资源文件

- `assets/colors.json.gz`
  - 来源：RinaChanBoard 上位机/小程序颜色表。
  - 内容：颜色名称与 RGB 十六进制值。
  - 用途：WebUI 进入页面后按需加载并填充颜色预设下拉框。

- `assets/unity/manifest.json.gz`
  - 来源：Unity 上位机 `Assets/Resources/Database` 数据库。
  - 内容：`VoiceDb`、`MusicDb`、`VideoDb`、`FaceModuleDb` 的索引和脸部部件矩阵。
  - 用途：只保存索引、部件矩阵和每条时间轴文件路径，不把所有帧序列一次性塞进 HTML。

- `assets/unity/voice/*.json.gz`
  - 内容：VoiceTimeLineDb 的帧序列。
  - 每次只加载当前选择的一条语音时间轴。

- `assets/unity/music/*.json.gz`
  - 内容：MusicTimeLineDb 的帧序列。
  - 每次只加载当前选择的一条歌曲时间轴。

- `assets/unity/video/*.json.gz`
  - 内容：VideoTimeLineDb 的帧序列。
  - 每次只加载当前选择的一条影像时间轴。
...
- 未导入 `.ogg` 音频文件。
- 未导入 `.mp4` 视频文件。
- 未导入 Unity 工程媒体资源、贴图、封面、meta 文件或编辑器文件。
- WebUI 现在只使用帧序列驱动 LED 矩阵，不再依赖内置媒体文件。

### WebUI 行为

- 启动页面时只加载 HTML 和小型颜色表。
- 进入/使用 Unity 帧序列页面时加载 `manifest.json.gz`。
- 选择某个 voice/music/video 项目时，只加载对应的单个 `.json.gz` 帧序列。
- 切换项目、停止播放、离开 Unity 页面或刷新页面时，释放当前帧序列引用。
- `window.RINA_UNITY_DB.voiceTimelines/musicTimelines/videoTimelines` 只保存当前正在使用的一条时间轴，避免所有时间轴常驻浏览器内存。

### 固件侧静态文件支持

- 继续使用已有静态文件 fallback 处理 `/assets/...` 请求。
- `.json.gz` 会作为 gzip 编码 JSON 发送给浏览器。
- 静态文件仍采用分块发送，避免 ESP32-S3 一次性读入大文件。

### 资源统计

- 颜色预设：53 项。
- Unity face modules：93 项。
- Voice 帧序列：143 条。
- Music 帧序列：8 条。
- Video 帧序列：7 条。
- 总计新增压缩资源文件：约 40 KB。
...
- 固件协议版本更新为：`1.6.5-esp32s3-native-asset-bundles`。
- WebUI 页面版本更新为：`1.6.3-assets-lazy`。

---

## v1.6.4 Wi-Fi / Open AP 修复外置注释

### 修改目标

- `wifi_config.py` 中 `AP_PASSWORD = ""` 时，ESP32-S3 热点明确配置为开放 AP：`authmode=0`，且不向 `WLAN.config()` 传入空密码。
- `AP_PASSWORD` 设置为 8–63 位字符串时，ESP32-S3 热点启用 WPA2-PSK：`authmode=3`。
- 不再使用内置默认密码 `rinachan` 覆盖空密码，避免用户明明配置为空密码但手机仍要求输入密码。
- STA 连接也按密码是否为空分支：空密码调用 `sta.connect(ssid)`，非空密码调用 `sta.connect(ssid, password)`。

### `wifi_config.py`

| 项目 | 新行为 |
|---|---|
| `WIFI_PASSWORD = ""` | 作为开放路由器 STA 连接处理。 |
| `AP_PASSWORD = ""` | 作为开放热点处理。 |
| `AP_PASSWORD = "8-63 chars"` | 作为 WPA2-PSK 热点处理。 |
| `AP_AUTHMODE = 0` | 自动/开放；当 `AP_PASSWORD` 非空时固件仍会启用 WPA2-PSK。 |
...
| 函数 | 修改 |
|---|---|
| `_start_wifi()` | 删除默认密码覆盖逻辑；AP 密码为空时强制 open AP；AP 密码非空时默认 WPA2-PSK。 |
| `_start_wifi()` | 增加 AP 密码长度保护：1–7 位或超过 63 位时打印日志并退回开放 AP，避免 WLAN 配置失败导致网络不可用。 |
| `_start_wifi()` | STA 连接根据 `WIFI_PASSWORD` 是否为空分别调用 `connect(ssid)` / `connect(ssid, password)`。 |
| `_api_wifi_status()` | 返回 `ap_security` 字段，WebUI 可显示当前热点是 `OPEN` 还是 `WPA2-PSK`。 |
| `_api_wifi_save()` | 保存 AP 密码前验证长度；空密码保存为开放 AP，非空密码保存为 WPA2-PSK。 |

### `webui_index.html.gz`

- Wi-Fi 状态区域显示 AP 安全模式。
- 热点密码输入框保留“留空=开放热点”逻辑。
- 保存前增加前端校验：热点密码必须为空，或为 8–63 位。

### 版本

- 协议版本：`1.6.5-esp32s3-native-asset-bundles`
- WebUI 标题：`1.6.4-open-ap-wifi-fix`


## v1.6.5 Assets 多文件 Bundle 按需加载外置注释

### 修改目标

本次修改把原本大量分散的 Unity 帧序列 asset 文件整合为更少的文件，但仍保留多个文件，避免把所有资源塞进一个超大 JSON。这样可以减少 ESP32-S3 WebUI 加载时的 HTTP 请求数量，同时保持按需加载和按需释放。

### Asset 文件布局

| 路径 | 作用 |
|---|---|
| `assets/colors.json.gz` | 颜色预设表，保持为单独文件。 |
| `assets/unity/manifest.json.gz` | Unity asset 索引、脸部模块表、voice/music/video 列表、bundle 映射表。 |
| `assets/unity/voice_pack_00.json.gz` ~ `voice_pack_05.json.gz` | 语音帧序列分组包，每包最多约 24 条 voice timeline。 |
| `assets/unity/music_pack.json.gz` | 所有 music timeline 的帧序列包。 |
| `assets/unity/video_pack.json.gz` | 所有 video timeline 的帧序列包。 |
| `assets/ASSET_BUILD_SUMMARY.json` | 构建统计，包括旧文件数量、新文件数量、bundle 数量和压缩大小。 |

### WebUI 加载逻辑

- WebUI 启动时只加载颜色表；进入 Unity 页面时才加载 Unity manifest。
- 用户选择具体 Unity 项目时，WebUI 从 manifest 读取该项目的 `bundle` 和 `bundleKey`。
- WebUI 只下载该项目所在的 pack 文件，例如 `voice_pack_03.json.gz`。
- pack 文件解压后只把当前选中的 timeline 挂到 `window.RINA_UNITY_DB.voiceTimelines/musicTimelines/videoTimelines`。
- 切换项目、停止播放、离开 Unity 页面或刷新页面时，会清空当前 timeline 和当前 bundle 引用，使浏览器可以释放内存。

### 保留的限制

- 仍然只导入 Unity 数据库中的帧序列 JSON。
- 仍然不导入 `.ogg`、`.mp4`、音频、视频、Unity meta、封面图或贴图资源。
- ESP32-S3 端仍然只负责静态分块发送 `.json.gz` 文件，不在 MicroPython 端解压或解析这些大型资源。

### 版本标记

- 固件协议版本：`1.6.5-esp32s3-native-asset-bundles`
- WebUI asset loader：`1.6.5-asset-bundles`
- 固件 banner：`RinaChanBoard ESP32-S3 370LED native WebUI 1.6.5 asset bundles ...`


---

## v1.6.6 TXS0108E / WS2812 一帧毛刺修复说明

### 修改范围

- `board.py`
- `board_370.py`
- `rina_protocol.py` / `main.py` / `esp32s3_network.py` 版本号字符串

### 修复原因

LED 数据线经过 TXS0108E 自动方向电平转换器时，ESP32-S3 的 NeoPixel/RMT 写帧会在 GPIO 所有权切换瞬间产生极短边沿。直接驱动 WS2812 时这个边沿通常不足以被锁存，但 TXS0108E 的自动方向/边沿加速结构可能把它放大成 LED 端可见的数据脉冲，导致偶发“一帧乱码”。

### 固件修复点

1. `LED_WRITE_RESET_GAP_US` 从 `280` 增加到 `500`。
2. 新增 `LED_PRE_WRITE_IDLE_LOW_US = 50`。
3. 新增 `_force_led_signal_low()`，每次写帧前显式把 GPIO2 拉低。
4. `_write_np_safely()` 改成：长 reset gap -> GPIO 低电平确认 -> 50 us 稳定等待 -> `np.write()` -> 长 reset gap。
5. `clear(write=True)` 不再直接调用 `np.write()`，统一走 `_write_np_safely()`。
6. `board_370.py` fallback 硬件层同步加入同样保护，避免备用导入路径绕过修复。

### 未启用的方案

没有启用“双写每帧”方案，因为 370 颗 LED 每帧传输时间约 11 ms，双写会明显降低动画刷新率。当前版本优先采用低成本的 reset gap + idle-low 固件修复。硬件长期方案仍然是把 TXS0108E 的 LED 数据通道替换为单向 HCT/AHCT 缓冲器。

## 1.6.7 Wi-Fi Split-Permission Fix

本版修复 Wi-Fi 页面权限逻辑：不再使用单一 `can_configure` 锁死全部字段，而是由 `/api/wifi/status` 返回 `from_ap`、`can_configure_sta`、`can_configure_ap`。通过 AP 热点访问时只允许修改路由器 STA 配置，AP 热点设置只读；通过路由器 STA IP 访问时只允许修改 AP 热点配置，路由器 Wi-Fi 配置只读。

`/api/wifi/save` 同步改为互斥保存：AP 客户端提交时只接收 `ssid/password` 并回填当前 AP 配置；STA 客户端提交时只接收 `ap_ssid/ap_password/ap_channel` 并回填当前 STA 配置，避免远程误改导致设备掉线或修改当前连接中的热点。

---

## 1.6.8 Wi-Fi Scan Dropdown Fix

本版修复 Wi-Fi 扫描结果的前端交互。原版将 `wifiScanResults` 放在 SSID 输入框下方的独立 `div` 中，视觉上与输入框脱节，并且选择后必须重新扫描才能再次查看结果。本版将扫描结果移动到 `wifiSsidWrap` 容器中，由相对定位容器承载绝对定位 dropdown。

### 修改范围

- `webui_index.html.gz`
  - HTML：SSID 输入框和 `wifiScanResults` 现在包在 `wifiSsidWrap` 中。
  - CSS：新增 `#wifiSsidWrap`、`#wifiScanResults`、`.scanItem`、`.scanBars`、`.scanSsid`、`.scanMeta` 等样式。
  - JS：新增 `_scanCache` 与 `_dropdownOpen`，扫描结果缓存后通过 `_renderDropdown()` 渲染。
  - JS：`wifiScan()` 只负责扫描并刷新缓存；`wifiPickNetwork()` 选择 SSID 后关闭 dropdown 并聚焦密码框。
  - JS：SSID 输入框 `focus` 时可展开已有缓存，`input` 时实时过滤，点击外部自动关闭。

### 交互结果

- 点击“扫描”后，网络列表直接挂在 SSID 输入框正下方。
- 每行显示信号强度图标、SSID、锁图标、RSSI 和认证方式。
- hover 有高亮反馈，当前输入框中已选 SSID 会显示 selected 高亮。
- 再次聚焦 SSID 输入框时，无需重新扫描即可展开最近一次扫描缓存。
- 输入 SSID 时会实时过滤缓存结果。

### 版本标记

- 固件协议版本：`1.6.9-esp32s3-native-unity-empty-key-fix`
- 固件 banner：`RinaChanBoard ESP32-S3 370LED native WebUI 1.6.9 Unity empty key fix ...`



## 1.6.9 Unity Empty Key Fix

### 问题

WebUI 在 Unity 帧序列页面中，如果用户在 `unityMediaSelect` 尚未由 `manifest.json.gz` 填充完成时点击“载入/预览”或“播放帧序列”，旧代码会读取到空字符串 key，并报错：`找不到时间轴项目：voice/`。

### 修复

- 新增 `itemKey(kind, item)` 和 `normalizeItemKey(kind, key)`，统一 voice/music/video 的选项 key 规则。
- `loadSelectedTimeline()` 在 key 为空时会自动填充下拉列表，并默认选择当前分类的第一项。
- `fillMediaItems()` 现在会在没有旧选择时显式 `selectedIndex = 0`，并输出当前选中项日志。
- `buildMediaTab()` 和安装流程会主动调用 `ensureMediaList()`，避免页面刚打开时 select 为空。
- 错误信息改为包含项目数量，方便判断是 manifest 为空还是 UI 还没初始化。

### 资源策略

仍保持 assets 多文件 bundle 按需加载：manifest 首次进入 Unity 页加载，具体 voice/music/video pack 只在选择项目时加载，停止或切换后卸载。
