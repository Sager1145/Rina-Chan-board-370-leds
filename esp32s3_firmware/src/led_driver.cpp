...
static inline void recordRefresh(uint32_t startUs, bool ok) {
    ...
    sLastRefreshUs = dur;
    if (dur > sMaxRefreshUs) sMaxRefreshUs = dur;
    if (!ok) ++sRefreshFail;
}
...
static esp_err_t rmtChannelCreate(size_t memSymbols) {
    ...
    if (err == ESP_OK) {
        ...
        if (err != ESP_OK && sChannel) { rmt_del_channel(sChannel); sChannel = nullptr; }
    }
    return err;
}
...
bool begin() {
    ...
    for (size_t i = 0; i < sizeof(candidates) / sizeof(candidates[0]); ++i) {
        ...
        if (mem == 0) continue;
        if (RINACHAN_LED_RMT_WITH_DMA) {
            if (mem > 2046) mem = 2046;   // DMA 硬上限 2047
            if (mem & 1U)   mem -= 1U;     // 必须偶数
        } else if (mem > 512) {
            continue;                       // 非 DMA 硬件块很小，跳过超大候选
        }

        const esp_err_t err = rmtChannelCreate(mem);
        if (err == ESP_OK) { chErr = ESP_OK; sActiveMemSymbols = mem; break; }
...
void setPixel(uint16_t physicalIndex, uint8_t r, uint8_t g, uint8_t b) {
    if (physicalIndex >= LED_COUNT) return;
...
bool refresh() {
    if (!sReady || !sChannel || !sEncoder) { ++sRefreshFail; return false; }
