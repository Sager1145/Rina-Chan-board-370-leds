...
bool RuntimeStore::initScrollFrameBuffer() {
    ...
    // The internal-SRAM fallback below keeps firmware functional on boards
    // without external RAM, but it is allocated lazily so the PSRAM path pays no
    // SRAM cost for it.
    ...
    // Keep scroll features available even when PSRAM allocation fails, but take
    // the SRAM cost only on that fallback path.  A one-time internal-SRAM heap
    // allocation replaces the former permanently-reserved static array, returning
    // ~140 KB of internal SRAM to the heap whenever PSRAM is present.  This is a
    // single boot-time allocation that is never freed, so it cannot fragment the
    // heap.  If both tiers fail the buffer simply stays unavailable and callers
    // guard on scrollFrameBufferReady().
    if (scrollFrameBits_ == nullptr) {
        ...
        if (scrollFrameBits_ == nullptr) {
            ...
            return false;  // scrollFrameBufferReady() stays false; scroll paths skip
        }
