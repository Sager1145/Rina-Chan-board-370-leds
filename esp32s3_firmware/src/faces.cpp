...
void setAutoInterval(uint32_t ms, bool persistSettings) {
    ...
    if (runtimeState().autoIntervalMs == nextInterval)
        return;
...
static int16_t findStartupDefaultFaceIndex() {
    ...
    for (uint16_t i = 0; i < runtimeAutoFaceCount(); ++i) {
        if (runtimeAutoFaces()[i].isStartupDefault)
...
static bool applyStartupDefaultFaceAfterScrollStop(bool restoreAutoMode) {
    ...
    if (!applySavedFaceIndex(static_cast<uint16_t>(defaultIndex), "firmware_text_scroll_stop_default_saved_face", playback))
        return false;
    runtimeState().lastAutoSwitchMs = millis();
    return true;
}
...
void serviceDeferredFaceRestore() {
    if (!runtimeState().deferredFaceRestoreActive)
        return;
    const uint32_t now = millis();
    if (!millisReached(now, runtimeState().deferredFaceRestoreDueMs))
        return;
...
void serviceAutoPlayback() {
    ...
    if (now - runtimeState().lastAutoSwitchMs < runtimeState().autoIntervalMs)
        return;
    runtimeState().lastAutoSwitchMs = now;
