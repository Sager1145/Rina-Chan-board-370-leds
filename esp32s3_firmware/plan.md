...
2. `src/main.cpp`
   - `MAX_SCROLL_FRAMES` 从 512 提升到 2048，适配最长 100 字符的逐列 M370 序列。
...
1. `data/index.html`
   ...
   - 新增 `preloadFirmwareRuntimeState()`，首次请求 `/api/status?summary=1`。
...
2. `src/web_api.cpp`
   ...
   - summary 模式下返回运行状态，但不返回 `renderer.lastM370` 和 `renderer.lit`。
