...
# matrix_demos.py
#
# Special matrix demo mode for the 370-LED hex matrix.
#
# Demo set:
# - rgb
# - fire
...
# Notes:
# - smooth 30 FPS
# - no checkerboard masking
# - animations may use the full matrix over time
# - each frame stays sparse enough to avoid filling the whole board
# - the hello-world scroll takes one full crossing over the set interval
# ---------------------------------------------------------------------------
...
class DemoController:
    ...
    def _rgb(self, t):
        _clear_all()
        # Single RGB demo: moving rainbow contour bands.
    ...
    def _hello_world(self, now_ms):
        ...
        # One full crossing of the full text over exactly interval_ms.
        ...
        # If the font is temporarily too sparse near the edges, add a very small
        # glowing leading dot so motion stays visible.
        if lit_count < 8:
            px = scroll_x - 1
            py = y0 + (_TEXT_H // 2)
            idx = logical_to_led_index(px, py)
